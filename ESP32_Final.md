# ESP32 Hidroponik Control Final
...
const unsigned long ONE_HOUR = 3600000; // 1 jam dalam ms (untuk testing: 60000 = 1 menit)
    ...
    if (!error && doc["success"] && doc["data"]["ppmThreshold"]) {
      ...
      if (newThreshold != ppmThreshold) {
        ...
        if (ppmThreshold == 800 && !harvestMode) {
          ...
          Serial.println("- Pompa akan ON jika PPM < 800");
...
void controlRelay() {
  if (harvestMode) {
    ...
    if (timerStarted) {
      // Cek apakah masih dalam periode 1 jam
      if (millis() - timerStartTime < ONE_HOUR) {
        // Dalam 1 jam: ON jika PPM < 800
        if (tdsValue < 800) {
        ...
        // 1 jam habis: relay permanen OFF
        timerStarted = false;
        digitalWrite(relayPin1, LOW);
        digitalWrite(relayPin2, LOW);
...
void updateDisplay() {
  ...
  lcd.setCursor(0, 1);
  if (harvestMode && timerStarted) {
    // Display sisa waktu mode panen
    unsigned long timeLeft = (ONE_HOUR - (millis() - timerStartTime)) / 1000;
...
- Control relay sesuai threshold baru
...
├─ PPM 350: Relay ON
├─ PPM 420: Relay OFF
└─ Control normal aktif
```

## Notes

- ONE_HOUR bisa diubah untuk testing:
  - 3600000 = 1 jam
  - 60000 = 1 menit
  - 10000 = 10 detik
