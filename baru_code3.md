# ESP32 Code dengan Dynamic PPM Threshold dari API
...
#include <ArduinoJson.h>  // Tambahan untuk parsing JSON
...
#define relayPin1 13
#define relayPin2 12
#define EEPROM_PPM_ADDR 0  // Alamat untuk menyimpan threshold di EEPROM
...
// API endpoints
const char* SENSOR_API = "https://9d69-103-224-73-240.ngrok-free.app/api/sensor-data";
const char* THRESHOLD_API = "https://9d69-103-224-73-240.ngrok-free.app/api/relay-control/ESP32_01";
...
void setup() {
  ...
  if (ppmThreshold < 0 || ppmThreshold > 2000) {
    ppmThreshold = 300;  // Reset ke default jika tidak valid
...
void loop() {
  // Check dan update threshold setiap interval
...
1. Penambahan Fitur:
   ...
   - Pengambilan PPM threshold dari API
   - Penyimpanan threshold di EEPROM
   - Update threshold otomatis setiap 30 detik

2. Endpoint API:

   - SENSOR_API: untuk mengirim data sensor
   - THRESHOLD_API: untuk mengambil threshold PPM

3. Kontrol Relay:
   - Menggunakan threshold dari API
   - Fallback ke nilai EEPROM jika API tidak tersedia
   - Logging status relay dan PPM yang lebih detail

## Cara Penggunaan

1. Upload kode ke ESP32
2. Pastikan backend sudah running dan di-ngrok
3. Update URL ngrok di kode sesuai dengan URL yang aktif
...
2. Jika relay tidak berfungsi:
   ...
   - Cek log Serial untuk status detail
